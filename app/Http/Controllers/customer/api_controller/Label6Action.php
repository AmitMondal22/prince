...
class Label6Action extends ResponceBaseController
{
    function add(Request $r): JsonResponse
    {
        try {
            ...
            $data = TdLabel6::create(["create_by" => auth()->user()->id,
                                        "l6_qty"=>$r->l6_qty,
    ...
    function edit(Request $r): JsonResponse
    {
        try {
            ...
            $data = TdLabel6::where("label6_id",$r->label6_id)->where("update_by",auth()->user()->id)->where("l6_stock","A")->where("l6_flag","A")->update(["create_by" => auth()->user()->id,
                                        "l6_qty"=>$r->l6_qty,
                                        "product_mastar_id"=>$r->product_mastar_id]);
